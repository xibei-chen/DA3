...
data_holdout[,"best_model_no_loss_pred"] <- rf_predicted_probabilities_holdout[,"fast_growing"]
RMSE(data_holdout[, "best_model_no_loss_pred", drop=TRUE], data_holdout$fast_growing) # 0.4595446
```

Since random forest is our best performing model, we also calculated its RMSE on the holdout set which equals to 0.46 just about the same as its CV RMSE in the training data. Furthermore, we also created a calibration curve for the model using its predictions for the holdout set and the actual probabilities. We can see on the plot below that in general it is relatively well calibrated in the middle. However, there are larger gap between the green line and the 45 degree orange line at the beginning and the end, indicating our prediction is not performing quite well in case of probabilities closer to 0 or 1.
...
The dataset covers two industry categories: Manufacturing (ind2 26-33), and Accommodation and Food Service Activities (ind2 55-56). As an extra task for pair work, we separated the two industries in holdout sample to random forest classification performance. Firstly we explored the difference between two samples in terms of the percentage of number of firms and share of fast growing firms. As shown in below graph, the number of firms in Accommodation and Food Service almost double the number of firms in Manufacturing. Furthermore, in Accommodation and Food Service, there are around 36% firms that are defined as fast growing. And in Manufacturing, fast growing firms make up about 41%. Therefore, despite there is quite large difference regarding the total number of firms in different industries, the share of fast growing firms are relative close.
...
data_ind <- data_holdout %>% mutate(ind= ifelse(data_holdout$ind2 %in% c('55','56'), 'a','m')) %>% group_by(ind,fast_growing_f) %>% count() %>% group_by((ind)) %>% mutate(percent= round(n/sum(n),2))

ggplot(data=data_ind, aes(x=ind, y=n, fill=fast_growing_f, label = percent))+
        geom_bar(position = position_stack(), stat = "identity", alpha=0.8)+
        geom_text(aes(label=percent), size = 5, vjust=0.9, color="white", position=position_stack(vjust=0.5)) +
        scale_fill_manual(values = c("#e85d04", "#2a9d8f"))+
        labs(y='Percentage of Number of Firms', x='a: Accommodation and Food Servie           m: Manufacturing')+
