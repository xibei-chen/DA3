...
geometry: "left=1.5cm,right=1.5cm,top=0.1cm,bottom=1cm"
output: pdf_document
---
...
* Predictor variables: Sex(male as base category), Marital status(married as base category), Race(white as base category), Highest degree(bachelor as base category), Age(squared and cubic), Weight(squared and cubic).
...
## Regression Analysis  - Predicting Hourly Wage
...
Build 4 Models using linear regression for target variable (hourly wage) with different predictors as follows:

1. age and age squared;
2. age, age squared, sex, and highest degree;
3. age, age squared, sex, highest degree, race, and marital status;
4. age, age squared, age cubic, sex, highest degree, race, marital status, weight, weight squared, weight cubic, interaction term of sex and age, interaction term of sex and highest degree.

For the full sample, Model4 is the best according to RMSE, while Model2 is the best according to BIC.
In terms of 5-fold Cross-validated RMSE, Model3 is the best, as we can see from below visual of the relationship between model complexity and performance.
...
```{r, echo=FALSE, fig.dim=c(4,3), fig.align='center', message=FALSE}
#######################################################
...
```{r, echo=FALSE, fig.dim=c(4,2.5), fig.align='center', message=FALSE}
...
kable(cv_mat, 
      "latex", booktabs = TRUE, 
      caption = '5-fold Cross Validated RMSE') %>% kable_styling(latex_options = c("hold_position","scale_down"))
# 5-fold Cross-validated RMSE: Model3 is the best
```
